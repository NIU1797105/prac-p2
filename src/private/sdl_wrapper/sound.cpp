...
void Sound_Callback(void* unused, Uint8* stream, int len)
{
    ...
    if (g_current_music != NULL)
    {
        if (g_current_music->ogg != NULL)
        {
            ...
            g_current_music->posicion_stream = stb_vorbis_get_sample_offset(g_current_music->ogg);
        ...
        if (sonido != NULL)
        {
            ...
            if (sonido->ogg != NULL)
            {
                ...
                if (sonido->estado == SOUND_STATE_PLAYING)
                {
                    ...
                    stb_vorbis_get_samples_short_interleaved(sonido->ogg, 2, g_sound_buffer_queue[var_sonido],
                                                             num_samples);
                }
            }
        }
